...
```{r}
library(tidyverse)
basic_inference_model_2 <- '
var model = function() {
   // normal prior of true value
   var value = gaussian(0, 1)

   // someone has observed a value of 2 with strong belief
   observe(Gaussian({mu: value, sigma: .5}), 2);
...
Infer(model)
'
posterior <- rwebppl::webppl(basic_inference_model_2)

posterior |>
   ggplot() + geom_density(aes(x=value)) + lims(x=c(-5,5), y=c(0, NA))
```
