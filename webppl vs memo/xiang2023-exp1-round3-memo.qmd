...
@jax.jit
def joint_utility_multiple_strength(strength_a, strength_b, effort_a, effort_b, include_gini):
    """
...
@jax.jit
def best_response_a(strength_a, strength_b, effort_b, include_gini):
    """
    ...
    utilities = jax.vmap(
        lambda ea: joint_utility_multiple_strength(strength_a, strength_b, ea, effort_b, include_gini=include_gini)[0]
    )(efforts)
    # else:
...
@jax.jit
def best_response_b(strength_a, strength_b, effort_a, include_gini):
    """
    ...
    # Multiple strength values
    utilities = jax.vmap(
        lambda eb: joint_utility_multiple_strength(strength_a, strength_b, effort_a, eb, include_gini=include_gini)[1]
    )(efforts)
    # else:
    #     # Single strength values
    #     utilities = jax.vmap(
...
def find_equilibrium(strength_a, strength_b, init_effort, include_gini, max_depth=10, verbose=False, trace=False):
    """
    ...
    def a(depth):
        ...
        effort_a = best_response_a(strength_a, strength_b, effort_b, include_gini)
    ...
    def b(depth):
        ...
        effort_b = best_response_b(strength_a, strength_b, effort_a, include_gini)
...
def starting_effort(posterior_sa, posterior_sb):
    ...
    for init_effort in efforts:
        stats = expected_joint_utility(init_effort, posterior_sa, posterior_sb, include_gini=True) # include gini coefficient because only computing a starting effort for joint effort model, which uses gini coef
...
def expected_joint_utility(init_effort, posterior_sa, posterior_sb, include_gini):
    """
    ...
    effort_a, effort_b, depth = find_equilibrium(sa, sb, init_effort, include_gini)

    # print(f"Strengths (A={sa:.2f}, B={sb:.2f}): Efforts (A={effort_a:.4f}, B={effort_b:.4f})")

    # Compute utilities and outcome at equilibrium
    utility_a, utility_b, success_prob = joint_utility_multiple_strength(sa, sb, effort_a, effort_b, include_gini=include_gini)

    # Test equilibrium finding with a sample strength pair
    if (init_effort == 0.0 or not include_gini) and DEBUG:
    ...
    return {
        ...
        'aE': effort_a,
        'bE': effort_b,
        'P': outcome2(sa, sb, effort_a, effort_b)
    }
...
def independent_effort_optimization(others_effort, posterior_sa, posterior_sb):
    ...
    utilities_a = jax.vmap(
        lambda ea: joint_utility_multiple_strength(
            sa, sb, ea, others_effort, include_gini=False
        )[0]  # Get utility_a
    )(efforts)
    ...
    utilities_b = jax.vmap(
        lambda eb: joint_utility_multiple_strength(
            sa, sb, others_effort, eb, include_gini=False
        )[1]  # Get utility_b
    )(efforts)
    ...
    utility_a, utility_b, success_prob = joint_utility_multiple_strength(
        sa, sb, effort_a, effort_b, include_gini=False
    )
...
class Model():
    ...
    def joint_effort(self):
        ...
        self.optimal_init_effort = starting_effort(self.posterior_sa, self.posterior_sb)
        if DEBUG:
        ...
        stats = expected_joint_utility(self.optimal_init_effort, self.posterior_sa, self.posterior_sb, include_gini=True) # include gini coef for joint model
