...
posterior_strengths = infer_strengths(0.0, 0.0, 1.0, 1.0)
...
def joint_utility_single_strength(strength_a, strength_b, effort_a, effort_b):
    ...
    """
    # Ensure that strength_a and strength_b are floats, not arrays
    if isinstance(strength_a, list) or isinstance(strength_b, list):
        raise ValueError("strength_a and strength_b must be single float values")
...
def joint_utility_multiple_strength(strength_a, strength_b, effort_a, effort_b):
    ...
    Returns: (utility_a, utility_b, success_prob)
    """
    # Ensure that strength_a and strength_b are arrays, not floats
    if not isinstance(strength_a, list) or not isinstance(strength_b, list):
        raise ValueError("strength_a and strength_b must be arrays of float values")
...
def starting_effort(posterior_sa_sb):
    """
    ...
    for init_effort in efforts:
        stats = expected_joint_utility(init_effort, posterior_sa_sb)
...
def expected_joint_utility(init_effort, posterior_sa_sb):
    """
    ...
    """
    # TODO: this is assuming posterior_sa_sb is point probabilities, but it should accept posterior samples like the webppl version does!
    ...
    for i, sa in enumerate(strength_values):
        for j, sb in enumerate(strength_values):
            ...
            if prob_sa_sb > 1e-10:  # Only compute for non-negligible probabilities
                ...
                effort_a, effort_b, _ = find_equilibrium(sa, sb, init_effort, verbose=True)
    ...
    if (init_effort == 0.0):
        ...
        print(f"Agent A equilibrium effort: {expected_agent_a_effort:.4f}")
        print(f"Agent B equilibrium effort: {expected_agent_b_effort:.4f}")
...
# convert from point probabilities to samples; there should be samples proportional to the posterior probabilities
posterior_samples = []
for i, sa in enumerate(strength_values):
    for j, sb in enumerate(strength_values):
        prob = posterior[i, j]
        if prob > 1e-10:
            num_samples = int(prob * 1000)  # Scale up for sample count (in theory, the minimum number of samples needed could be computed)
            posterior_samples.extend([(sa, sb)] * num_samples)

optimal_init_effort = starting_effort(posterior_samples)
...
stats = expected_joint_utility(optimal_init_effort, posterior)
