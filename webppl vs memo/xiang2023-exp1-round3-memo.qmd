...
def find_equilibrium(strength_a, strength_b, init_effort, max_depth=10, verbose=False):
    ...
    """
    effort_a = init_effort

    # Start depth at 1 to match WebPPL convention
    for depth in range(1, max_depth + 1):
        effort_b = best_response_b(strength_a, strength_b, effort_a)
        effort_a_new = best_response_a(strength_a, strength_b, effort_b)

        # Check convergence
        if np.abs(effort_a_new - effort_a) < 0.06:
            if verbose:
                print(f"  Converged at depth {depth}: effort_a={effort_a_new:.4f}, effort_b={effort_b:.4f}")
            return effort_a_new, effort_b, depth

        effort_a = effort_a_new
...
def expected_joint_utility(init_effort, posterior_sa, posterior_sb):
    ...
    effort_a, effort_b, depth = find_equilibrium(sa, sb, init_effort)
...
# TODO!: optimal_init_effort should be 0.5! may need to reimplement nash equilibrium part
