...
- TODO modify joint effort model to implement solitary effort and compensatory effort models
...
TODO Okay not sure why result is kinda unreliable; maybe need to ensure that the different methods' results aren't getting mixed up during caching. ALSO, not sure why P jumps to 1 so fast after F,F;F,F! **maybe I need to fully regenerate the results each time I change anything, since Quarto's caching might not be sufficient**