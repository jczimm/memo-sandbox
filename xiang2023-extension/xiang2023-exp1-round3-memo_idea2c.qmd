...
TODO: keep just one observes_event instance and test that the results are the same if we replaced it with the original code (to test that the marginalization that's happening due to the function calls isn't disrupting the model/e.g. adding new assumptions about what parameters are fixed)
