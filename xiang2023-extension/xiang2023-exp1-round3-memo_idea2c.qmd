...
alpha = .001 # 13.5  # effort cost coefficient, from original paper; kept fixed for replication
beta = 24.5   # inequality aversion coefficient, from original paper; kept fixed for replication
gamma = 1.5 # strength of belief that agent will achieve optimal effort (inverse temperature)
delta = 6. # strength of belief in perceived effort (inverse temperature)
...
GAMMA = 0
DELTA = 1
def ModelParams(gamma, delta):
    return np.array([ gamma, delta ])

@memo(save_comic="memo-comic-xiang-strength-inference-inner", debug_trace=DEBUG)
def likelihood_of_outcome(
    outcome, strength, perceived_effort, reward,
    modelParams: ...
):
    ...
    participant: thinks[
        ...
        agent: chooses(actual_effort in efforts,
            wpp=norm_pdf(actual_effort, opt_effort, exp(1 - {modelParams[GAMMA]}))),
        # and that the agent's visible effort is near the actual effort
        agent: given(visible_effort in efforts,
            wpp=norm_pdf(visible_effort, actual_effort, exp(1 - {modelParams[DELTA]})))
    ]
...
def infer_strengths[query_strength_a: strength_values, query_strength_b: strength_values](
    ...
    participant: observes_event(wpp=likelihood_of_outcome(r1_outcome_a, agent_a.strength, r1_perceived_effort_a, {low_reward}, modelParams))
    participant: observes_event(wpp=likelihood_of_outcome(r1_outcome_b, agent_b.strength, r1_perceived_effort_b, {low_reward}, modelParams))
    participant: observes_event(wpp=likelihood_of_outcome(r2_outcome_a, agent_a.strength, r2_perceived_effort_a, {high_reward}, modelParams))
    participant: observes_event(wpp=likelihood_of_outcome(r2_outcome_b, agent_b.strength, r2_perceived_effort_b, {high_reward}, modelParams))
...
```{python}
sample_strengths = infer_strengths(0., 0., 0., 0., .0, .0, .4, .4, ModelParams(gamma=2., delta=1.))
...
# if we don't give any credence to perceived effort, if the person fails both times, they're most likely below the requisite strength
sample_strengths = infer_strengths(0., 0., 0., 0., .5, .0, .5, .0, ModelParams(gamma=1., delta=.0001))
axes[0].plot(strength_values, np.sum(sample_strengths, axis=1))
axes[0].set_xlabel('Agent Strength')
axes[0].set_ylabel('Posterior Probability')
axes[0].set_title('P(strength | FAIL)')
# axes[0].set_ylim(0, 0.008)

# if we believe somewhat in perceived effort, if the person fails both times, since the person still looks to have put in full effort, we have reason to believe they have strength 5
# NOTE: setting gamma to ~0 does not prevent the model from incorporating the information about the outcome! it just makes the model less certain that effort is intended to be optimal (conditioning on the outcome is not affected by gamma)
sample_strengths = infer_strengths(0., 0., 0., 0., 1., .0, 1., .0, ModelParams(gamma=1., delta=3.7))
axes[1].plot(strength_values, np.sum(sample_strengths, axis=1))
axes[1].set_xlabel('Agent Strength')
axes[1].set_ylabel('Posterior Probability')
axes[1].set_title('P(strength | FAIL, perceived effort = 100%)')
# axes[1].set_ylim(0, 0.008)

# if perceived effort is 50%, then maybe they are strong enough (closer to strength 10)!
sample_strengths = infer_strengths(0., 0., 0., 0., .5, .0, .5, .0, ModelParams(gamma=1., delta=3.7))
axes[2].plot(strength_values, np.sum(sample_strengths, axis=1))
axes[2].set_xlabel('Agent Strength')
axes[2].set_ylabel('Posterior Probability')
axes[2].set_title('P(strength | FAIL, perceived effort = 50%)')
# axes[2].set_ylim(0, 0.008)

plt.tight_layout()
plt.show()

# NOTE: easier to interpret when ignoring effort cost
```
...
class Model():
    def __init__(self, r1_outcome_a, r1_outcome_b, r2_outcome_a, r2_outcome_b):
        ...
        posterior_strengths = infer_strengths(r1_outcome_a, r1_outcome_b, r2_outcome_a, r2_outcome_b, np.array([ gamma, delta ]))
